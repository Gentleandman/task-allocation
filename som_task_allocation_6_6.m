...
A=zeros(1,6);
B=cell(6,1);                                                               %用于寄存target和AUV的距离  
for Targets_number=1:6                                                                  %target的次序
    for AUVs_number=1:6                                                             %AUV的次序
...
B1=B;
 ...
 for Targets_number=1:6                                                   %功能:target按此次顺序，产生一组对应的配对
  ...
  for N=1:1
      ...
      for k=1:In_DNumber
          for AUVs_number=1:6    
              if max(max(X_AUV1(AUVs_number),Y_AUV1(AUVs_number)),Z_AUV1(AUVs_number))>1000
              continue
              end
      ...
      Pick_inD=zeros(1,6);                                               %判定是否在邻域内的矩阵
      for i=1:Targets_number-1
              for kk=1:1
                  Pick_inD(Choosn_AUV_number(i,kk))=1000;
              end
      end
      for K=1:1
          Pick_inD(Choosn_AUV_number(Targets_number,K))=1000;%把已经挑选好的AUV重新写进pick矩阵，防止被重新挑选
          for i=1:10
...
plot(P)
toc
%X_AUV=100*rand(6,1)';Y_AUV=100*rand(6,1)';Z_AUV=100*rand(6,1)';X_Target=100*rand(6,1)';Y_Target=100*rand(6,1)';Z_Target=100*rand(6,1)';